class SeatingPlan {
    ...
    createStudentCard(student) {
        ...
        // Add hover events for portal-based popup system
        card.addEventListener('mouseenter', (e) => {
            this.showStudentActionsPortal(card, actions);
        });
        card.addEventListener('mouseleave', (e) => {
            // Add small delay to allow moving to popup
            this.hideTimeout = setTimeout(() => {
                this.hideStudentActionsPortal();
            }, 100);
        });
    ...
    showStudentActionsPortal(studentCard, originalActions) {
        ...
        const portal = document.getElementById('ui-top-layer');
        
        // Remove any existing popup
        const existingPopup = portal.querySelector('.popup-portal');
        if (existingPopup) {
        ...
        // Create popup in portal
        const popup = document.createElement('div');
        popup.className = 'popup-portal';
        
        // Clone the buttons from original actions
        const buttons = originalActions.querySelectorAll('button');
        buttons.forEach(button => {
            const clonedButton = button.cloneNode(true);
            // Rebind event listeners since cloneNode doesn't copy them
            this.rebindButtonEvents(clonedButton, button);
            popup.appendChild(clonedButton);
        });
        
        // Position popup above the card, centered horizontally
        const popupWidth = 130; // Approximate width
    ...
    hideStudentActionsPortal() {
        const portal = document.getElementById('ui-top-layer');
        const popup = portal.querySelector('.popup-portal');
        if (popup) {
            popup.remove();
        }
        if (this.hideTimeout) {
    ...
    rebindButtonEvents(clonedButton, originalButton) {
        ...
        // Rebind click events based on button class/type
        if (clonedButton.classList.contains('btn-edit')) {
            const studentId = originalButton.closest('.student-card')?.dataset.studentId;
            if (studentId) {
                clonedButton.addEventListener('click', (e) => {
                    e.stopPropagation();
                    this.editStudent(parseInt(studentId));
                    this.hideStudentActionsPortal();
                });
            }
        } else if (clonedButton.classList.contains('btn-lateness')) {
            const studentId = originalButton.closest('.student-card')?.dataset.studentId;
            if (studentId) {
                clonedButton.addEventListener('click', (e) => {
                    e.stopPropagation();
                    this.openLatenessModal(parseInt(studentId));
                    this.hideStudentActionsPortal();
                });
            }
        } else if (clonedButton.classList.contains('btn-reset')) {
            const studentId = originalButton.closest('.student-card')?.dataset.studentId;
            if (studentId) {
                clonedButton.addEventListener('click', (e) => {
                    e.stopPropagation();
                    this.resetStudentCounter(parseInt(studentId));
                    this.hideStudentActionsPortal();
                });
            }
        }
